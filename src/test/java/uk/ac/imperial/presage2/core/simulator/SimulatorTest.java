...
abstract public class SimulatorTest {
	...
	public void testSimulator() {
		...
		final Participant part1 = context.mock(Participant.class, "participant1");
		final Participant part2 = context.mock(Participant.class, "participant2");
		...
		context.checking(new Expectations() {{
			allowing(scenario).getParticipants(); will(returnValue(partSet));
			allowing(scenario).getPlugins(); will(returnValue(plugSet));
			allowing(scenario).getTimeDriven(); will(returnValue(tdSet));
			allowing(time).increment();
			allowing(finishTime).clone(); will(returnValue(finishTime));
		...
		context.checking(new Expectations() {{
			allowing(scenario).getFinishTime(); will(returnValue(finishTime));
			exactly(2).of(finishTime).greaterThan(time); will(returnValue(true)); inSequence(loopLimit);
			oneOf(finishTime).greaterThan(time); will(returnValue(false)); inSequence(loopLimit);
